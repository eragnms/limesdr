...
void Analyser::add_data(std::vector<float> data)
{
        m_data.clear();
...
void Analyser::add_data(std::vector<std::complex<float>> data)
{
        m_data.clear();
        m_data.set_size(data.size());
        m_data = arma::conv_to<arma::cx_vec>::from(data);
...
void Analyser::add_data(std::vector<std::complex<int16_t>> data)
{
        m_data.clear();
        m_data.set_size(data.size());
...
void Analyser::add_data(arma::cx_vec data)
{
        m_data.clear();
...
void Analyser::plot_real_data()
{
        plot(arma::real(m_data), "real part");
}

void Analyser::plot_imag_data()
{
        plot(arma::imag(m_data), "imag part");
}

void Analyser::plot_data()
{
        if (m_data.n_rows > 0) {
...
void Analyser::plot(std::vector<float> y, std::string title)
{
        Gnuplot g1("lines");
        g1.set_title(title);
        g1.plot_x(y);
...
void Analyser::plot(std::vector<float> y)
{
        plot(y, "");
}

void Analyser::plot(arma::vec y, std::string title)
{
        std::vector<float> y_p = arma::conv_to<std::vector<float>>::from(y);
        plot(y_p, title);
}

void Analyser::plot(arma::vec y)
{
        plot(y);
}

void Analyser::save_data(std::string filename)
{
        std::string file;
